...
class asyncHttp:
    ...
    def add(self, api: str, data: dict = None) -> None:
        ...
        asyncio.run_coroutine_threadsafe(
            self._asynclink(api, data), self._loop)
    ...
    def download_file(self, file_name: str, file_url: str) -> None:
        """异步图片下载"""
        asyncio.run_coroutine_threadsafe(
            self._download_file(file_name, file_url), self._loop)
    ...
    def apiLinkError(self, err_json: dict) -> None:
        ...
        cqapi发生错误
        """
        logging.error("api 发生错误 %s: %s code: %s" % (
            err_json["msg"], err_json["wording"], err_json["retcode"]))
